...
app.post('/api/cases/:id/session-entry', auth, async (req, res) => {
  const { session_date, decision, next_session_date } = req.body;
  ...
  await pool.query('UPDATE cases SET session_date=$1, decision=$2, postpone_date=$3, updated_at=NOW() WHERE id=$4',
    [session_date, decision||null, next_session_date||null, req.params.id]);
...
async function resyncCaseFromSessions(caseId) {
  ...
  await pool.query('UPDATE cases SET session_date=$1, decision=$2, updated_at=NOW() WHERE id=$3',
