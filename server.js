...
async function initDB() {
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS procedure_postponements (
      id SERIAL PRIMARY KEY, procedure_id INTEGER REFERENCES procedures(id) ON DELETE CASCADE,
      old_date TEXT, new_date TEXT, changed_by INTEGER, changed_by_name TEXT,
      created_at TIMESTAMP DEFAULT NOW()
    );
  `);
...
app.put('/api/procedures/:id/update-status', auth, async (req, res) => {
  ...
  if (new_status === 'مؤجل' && new_date) {
    ...
    if ((oldDate||'') !== (new_date||'')) {
      await pool.query('INSERT INTO procedure_postponements (procedure_id,old_date,new_date,changed_by,changed_by_name) VALUES ($1,$2,$3,$4,$5)',
        [req.params.id, oldDate||null, new_date, req.user.id, req.user.full_name]);
    }
  }
...
app.post('/api/restore', auth, adminOnly, async (req, res) => {
  ...
  try {
    ...
    for (const p of (b.procedure_postponements||[])) {
      await client.query(
        'INSERT INTO procedure_postponements (id,procedure_id,old_date,new_date,changed_by,changed_by_name,created_at) VALUES ($1,$2,$3,$4,$5,$6,$7)',
        [p.id,p.procedure_id,p.old_date,p.new_date,p.changed_by,p.changed_by_name,p.created_at]);
    }
