...
app.get('/api/backup', auth, adminOnly, async (req, res) => {
  try {
    const [usersR, employeesR, casesR, proceduresR, logsR, waR, settingsR, auditR, sessionsR] = await Promise.all([
...
app.post('/api/restore', auth, adminOnly, async (req, res) => {
  ...
  try {
    ...
    const seqFixes = [
      ...
      ['audit_log','audit_log_id_seq'], ['case_sessions','case_sessions_id_seq']
    ];
