...
async function initDB() {
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS case_sessions (
      id SERIAL PRIMARY KEY, case_id INTEGER REFERENCES cases(id) ON DELETE CASCADE,
      session_date TEXT NOT NULL, decision TEXT, notes TEXT, next_session_date TEXT,
...
// CASE SESSIONS (hearing history: date + ruling/decision per session)
app.get('/api/cases/:id/sessions', auth, async (req, res) => {
  const r = await pool.query(
    `SELECT s.*, u.full_name as created_by_name FROM case_sessions s
     LEFT JOIN users u ON s.created_by=u.id WHERE s.case_id=$1 ORDER BY s.session_date DESC, s.created_at DESC`,
    [req.params.id]);
  res.json(r.rows);
});
app.post('/api/cases/:id/sessions', auth, async (req, res) => {
  const { session_date, decision, notes, next_session_date } = req.body;
  if (!session_date) return res.status(400).json({ error: 'تاريخ الجلسة مطلوب' });
  const r = await pool.query(
    'INSERT INTO case_sessions (case_id,session_date,decision,notes,next_session_date,created_by) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id',
    [req.params.id, session_date, decision||null, notes||null, next_session_date||null, req.user.id]);
  const cn = await pool.query('SELECT case_number FROM cases WHERE id=$1', [req.params.id]);
  await logAudit(req.user, 'إضافة جلسة', 'قضية', req.params.id, cn.rows[0]?.case_number);
  res.status(201).json({ id: r.rows[0].id });
});
...
app.get('/api/cases/:id/timeline', auth, async (req, res) => {
  try {
    ...
    const sessionsR = await pool.query(
      `SELECT s.*, u.full_name as created_by_name FROM case_sessions s
       LEFT JOIN users u ON s.created_by=u.id WHERE s.case_id=$1 ORDER BY s.created_at ASC`, [caseId]);
    ...
    auditR.rows.forEach(a=>{
      events.push({ type:a.action, timestamp:a.created_at, description:'تعديل بيانات القضية', user:a.user_name });
    });
    waR.rows.forEach(w=>{
      events.push({ type:'واتساب', timestamp:w.sent_at, description:`إشعار (${w.type}) للموكل ${w.client}`, user:w.sent_by });
    });
    sessionsR.rows.forEach(s=>{
      let desc = `جلسة بتاريخ ${s.session_date}`;
      if(s.decision) desc += ` — القرار: ${s.decision}`;
      if(s.next_session_date) desc += ` — الجلسة القادمة: ${s.next_session_date}`;
      if(s.notes) desc += ` — ${s.notes}`;
      events.push({ type:'جلسة', timestamp:s.created_at, description:desc, user:s.created_by_name });
    });
...
app.post('/api/restore', auth, adminOnly, async (req, res) => {
  ...
  try {
    ...
    for (const s of (b.case_sessions||[])) {
      await client.query('INSERT INTO case_sessions (id,case_id,session_date,decision,notes,next_session_date,created_by,created_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)',
        [s.id,s.case_id,s.session_date,s.decision,s.notes,s.next_session_date,s.created_by,s.created_at]);
    }
