...
app.put('/api/cases/:id', auth, async (req, res) => {
  ...
  // detect exactly what changed on decision/dates/status and log it in detail —
  // this is what powers the automatic case history (سجل القضايا), no manual entry needed
  const changes = [];
  if (b) {
    if ((b.decision||'') !== (decision||'')) changes.push(`القرار: "${b.decision||'—'}" ← "${decision||'—'}"`);
    if ((b.session_date||'') !== (session_date||'')) changes.push(`تاريخ الجلسة: ${b.session_date||'—'} ← ${session_date||'—'}`);
    if ((b.postpone_date||'') !== (postpone_date||'')) changes.push(`تاريخ التأجيل: ${b.postpone_date||'—'} ← ${postpone_date||'—'}`);
    if ((b.status||'') !== (status||'')) changes.push(`الحالة: ${b.status||'—'} ← ${status||'—'}`);
  }
  const label = (b?.case_number||'') + (changes.length ? ' — '+changes.join(' | ') : ' — تعديل بيانات عامة');
  await logAudit(req.user, 'تعديل', 'قضية', req.params.id, label);
  res.json({ message: 'تم التحديث' });
});
